...
@section('container')
    <div class="pt-3 pb-2 mb-3 container overflow-auto">
        ...
        <div class="row my-4">
            <div class="col-lg-8">
                <form method="post" action="/hikamin/anime/{{ $anime->slug }}" class="mb-5" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <div class="mb-3">
                    <label for="bg" class="form-label fs-5 w-100">Potrait Background</label>
                        <input type="hidden" name="old_img" value="{{ $anime->img }}">
                    ...
                    <label for="img_bg" class="form-label fs-5 w-100">Landscape Background</label>
                        <input type="hidden" name="old_img_bg" value="{{ $anime->img_bg }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" required autofocus value="{{ old('judul')? old('judul'):$anime->judul }}" />
                        @error('judul')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('alter_judul') is-invalid @enderror" id="alter_judul" name="alter_judul" required autofocus value="{{ old('alter_judul')? old('alter_judul'):$anime->alter_judul }}" />
                        @error('alter_judul')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" required value="{{ old('slug')? old('slug'):$anime->slug }}" />
                        @error('slug')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('produser') is-invalid @enderror" id="produser" name="produser" required value="{{ old('produser')? old('produser'):$anime->produser }}" />
                        @error('produser')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('studio') is-invalid @enderror" id="studio" name="studio" required value="{{ old('studio')? old('studio'):$anime->studio }}" />
                        @error('studio')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="row">
                            <div class="col-lg d-flex flex-wrap">
                                ...
                                <div class="d-flex flex-grow-1 m-1">
                                    <input type="checkbox" name="genre[]" class="genre-checkbox" id="btncheck{{ $genre->slug }}" value="{{ $genre->nama }}" {{ (is_array(old('genre')) and in_array($genre->nama, old('genre')) or in_array($genre->nama, $old_genre))? ' checked':'' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('skor') is-invalid @enderror" id="skor" name="skor" required value="{{ old('skor')? old('skor'):$anime->skor }}" />
                        @error('skor')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required value="{{ old('tipe')? old('tipe'):$anime->tipe }}" />
                        @error('tipe')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('status') is-invalid @enderror" id="status" name="status" required value="{{ old('status')? old('status'):$anime->status }}" />
                        @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('total_episode') is-invalid @enderror" id="total_episode" name="total_episode" required value="{{ old('total_episode')? old('total_episode'):$anime->total_episode }}" />
                        @error('total_episode')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('durasi') is-invalid @enderror" id="durasi" name="durasi" required value="{{ old('durasi')? old('durasi'):$anime->durasi }}" />
                        @error('durasi')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="date" class="form-control @error('tanggal_rilis') is-invalid @enderror" id="tanggal_rilis" name="tanggal_rilis" required value="{{ old('tanggal_rilis')? old('tanggal_rilis'):date('Y-m-d', strtotime($anime->tanggal_rilis)) }}" />
                        @error('tanggal_rilis')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" id="" class="form-control @error('deskripsi') is-invalid @enderror" required>{{ old('deskripsi')? old('deskripsi'):$anime->deskripsi }}</textarea>
                        @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <a href="/hikamin/anime" class="btn btn-danger" role="button">Cancel</a>
                    <button type="submit" class="btn btn-primary mx-2">Edit Post</button>
                </form>
            </div>
        </div>
    </div>
